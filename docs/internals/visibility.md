...
A note is gated before the Notes screen widens it. `Note` is the creator's working record, so `notes.list` and `findById` take the creator proof and answer a player `NotFound` even for a shared note; a player reads `playerNotes` (`GET …/player-notes`), the shared notes as `PlayerNote` with no visibility or provenance. Gating it before the Notes screen gains fields is the point: a field added to `Note` does not reach a player until somebody adds it to `PlayerNote` on purpose. A player gets a note's `category` and never its pin; a player's recap reads its read-alouds through the same select list (`playerNoteColumns`). `apps/server/test/note-player.test.ts` reads the raw wire.
