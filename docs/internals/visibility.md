...
What `shared` means for a row is `sharedWithPlayers`, the one fragment every row-level read test uses. For every table it is `visibility = 'shared'`, except `encounter`, which is shared only when it is also Ready (`encounter_prep.ready`): a draft stays the creator's whatever its Share switch says. The test is an `exists` over the creator-only prep row, so no player read selects the flag. Because it sits in the seam, every path to an encounter inherits it: its list and `findById`, the roster's containment chain, `lastPlayed`, and the name a fight goes by. A run carries its encounter's id and a snapshot of its name; `runColumns` (`repo/EncounterRuns.ts`, over `runEncounterReadable`) selects them only for a reader who may read that encounter and otherwise answers `encounterId: null` and "A fight", so sharing a fight never publishes an encounter the creator kept hidden or unfinished. A deleted template fails closed to the neutral name for players; the creator keeps the snapshot. `apps/server/test/encounter-drafts.test.ts` drives each combination with real actors.
