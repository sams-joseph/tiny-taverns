...
- A column that would be a second answer is worse than none. Three `Encounter` fields are computed per read in `repo/Encounters.ts` (`selectEncounters`), each over rows the actor can see, so the card and the roster agree: `creatureCount` is `sum(encounter_creature.count)`; `difficulty` is the DMG method over the roster's XP and the levels of the seated characters (`packages/api/src/EncounterDifficulty.ts`, the one implementation; `0059` dropped the band the DM used to pick); `lastPlayed` is the latest ended run, through `containedRowReadable` over `RUN`. The roster subqueries use `nestedRowReadableWithin`, which omits the parent check because the enclosing query already applied it; use it in a subquery over the parent table and nowhere else. A creature's XP is its stat block's `xp`, else the XP table's figure for its `cr` (`creatureXp`), else null, and a roster holding a null is unrated rather than guessed. `Encounter` is the creator's read; a player's `PlayerEncounter` carries none of these numbers, only the shared lines' names and counts and `lastPlayed` over the runs they can see (see [Visibility](visibility.md)).
...
A stored flag is right when no other column answers the question. `encounter_prep.ready` (`0061_encounter_ready.ts`) is the DM's word that an encounter is ready to run: a conversation has no roster and may be ready, and a full roster may still lack its tactics, so nothing computes it. It sits on the creator-only prep row, not on `encounter`, which a player reads when it is shared. Only a person sets it; the column default makes every new encounter a draft, Hob's accepted ones included.
