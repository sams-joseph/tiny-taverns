...
- A column that would be a second answer is worse than none. Three `Encounter` fields are computed per read in `repo/Encounters.ts` (`selectEncounters`), each over rows the actor can see, so the card, the roster and a player's answer agree and nothing hidden moves a number a player reads: `creatureCount` is `sum(encounter_creature.count)`; `difficulty` is the DMG method over the roster's XP and the levels of the seated characters (`packages/api/src/EncounterDifficulty.ts`, the one implementation; `0059` dropped the band the DM used to pick); `lastPlayed` is the latest ended run, through `containedRowReadable` over `RUN`. The roster subqueries use `nestedRowReadableWithin`, which omits the parent check because the enclosing query already applied it; use it in a subquery over the parent table and nowhere else. A creature's XP is its stat block's `xp`, else the XP table's figure for its `cr` (`creatureXp`), else null, and a roster holding a null is unrated rather than guessed.
