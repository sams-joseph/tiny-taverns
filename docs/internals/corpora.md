...
`usableInCampaign` (`repo/visibility.ts`) is what every campaign corpus read answers: rows in no campaign, gated on `campaignReadable`, that are (a) the bundle under the row-visibility rule `corpusRowReadable` always applied to it (creator, or `visibility = 'shared'`), (b) the reader's own Library, untested for visibility, or (c) originals shared to the campaign's Shared World via `group_library_share`. It never returns a campaign row. `creatures.list`, `options.list`, `repo/Search.ts`'s creature arm, `Characters.validateSubrace`, a seated character's sheet (`characterVocabulary`) and Hob's per-request vocabulary all compose it, so what is pickable is what validates, what search finds and what Hob is held to.
...
- **Races contain subraces; there is no fourth kind.** A subrace is a child in `RaceBody.subraces`, resolved everywhere through `subraceNamed(race, label)` (`packages/api/src/CharacterOption.ts`). `repo/Characters.ts` validates a named subrace against a usable race in the vocabulary of the write: the create's context campaign or the core rules, and on an edit the character's own (`characterVocabulary`); a free-text race may stand alone, and a stale subrace beside a changed race is a `Conflict`.
- **The core rules are the bundle's shared rows and nothing else** (`coreRulesUsable`, `GET /library/options/core`): the vocabulary of a character made with no campaign, and of any character's sheet while it sits at no table. It differs from the Library read by leaving out the account's own originals, and from a campaign's by leaving out Shared World shares, so a character with no campaign starts from exactly what every table's players can pick ([Characters](characters.md)).
