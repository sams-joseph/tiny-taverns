...
- **Visibility is exactly the character's.** The wire carries `Character.portrait` (three signed paths, or `null`) and `portraitPending`, and URLs are minted in `toCharacter`, the one mapper every character read goes through (`Characters`, and `Party`'s seat reads), after that read's own predicate. Whoever can load the character gets its picture; nobody else gets a URL. `portraitColumns` must be in every select that becomes a `Character`; `toCharacter` throws on a row without it. `portraits.test.ts` drives the owner, the creator, a seat-mate before and after the seat is shared, and a stranger.
...
- **The screens** use `CharacterPortrait` (`apps/web/src/characters/CharacterPortrait.tsx`) on the _My characters_ card and both sheet plates: initials always, the image over them when there is one, initials again on any error. While `portraitPending`, the card and sheet say Hob is drawing and re-read every two seconds for up to three minutes (`portraitPolling.ts`). The party list, runner and player table still draw their own plates.
