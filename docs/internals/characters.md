...
After a character is made through either composer, Hob draws it once (the captain's decisions: automatic, one time, owner only, no upload, no redraw, one house style). The trigger is `Portraits.drawAfterCreate` (`apps/server/src/portraits/Portraits.ts`), called by the two handlers that make a character, the form's create and Hob's accept, **after** their transaction commits; it is not inside `createOwn` because a job must find a committed row, and Hob's accept wraps `createOwn` in its own transaction. A third way of making a character must call it too.

- **The record** is `character_portrait` (`0048_character_portraits.ts`), one row per character ever (`unique (character_id)`), bound to the character and its owner by the composite key to `character (id, account_id)` and cascaded with it. It keeps the exact prompt, the model, the provider's token usage, the hash and the sizes, and a failed row names why: `refused`, `provider`, `timeout`, `interrupted`, `storage`, `skipped` (no race, class, appearance or background: `portraitHasSubject`) or `capped` (the daily caps). With portraits OFF nothing is recorded at all. It is in `NOT_CONTENT`: the character answers who may see it.
...
- **Visibility is exactly the character's.** The wire carries `Character.portrait` (three signed paths, or `null`) and `portraitPending`. Whoever can load the character gets its picture; nobody else gets a URL. Every URL is minted by `portraitImages` in `repo/Characters.ts`, and only for a portrait id that a read's own predicate returned. Two kinds of read return one:
...
- **Deleting the character deletes the files.** A trigger on `character_portrait` enqueues the row's `storage_prefix` into `storage_deletion` in the deleting transaction, whatever deleted it; the worker drains the outbox. A draw stores its files only inside a transaction that holds the portrait row's lock, so a delete mid-draw waits for the last put and its enqueued deletion comes after it.
- **The screens** use `CharacterPortrait` (`apps/web/src/characters/CharacterPortrait.tsx`) on the _My characters_ card and both sheet plates: initials always, the image over them when there is one, initials again on any error. While `portraitPending`, the card and sheet say Hob is drawing and re-read every two seconds for up to three minutes (`portraitPolling.ts`). The list rows (both party lists, the runner's PC rows, the player table's `you` and `ally`) use its 28px `row` size. With no portrait, a row keeps its existing icon (`fallback`) or, on the player table, draws nothing, so a server with portraits off looks as it did. Only the pending state is card-and-sheet; a list row shows a portrait from the next read after it is ready.
