...
`GET /portraits/:imageId/:variant?e=&s=`, `GET /campaign-images/:imageId/:variant?e=&s=` and `GET /shared-world-images/:imageId/:variant?e=&s=` (the `images` group, one endpoint per kind) are the only routes that serve bytes. They have no `Authorization`, because an `<img>` cannot send the bearer header. The capability is an HMAC over `{tag}:{imageId}:{variant}:{expiry}` (`images/ImageUrls.ts`, keyed by `PORTRAIT_URL_SECRET`). The tag is the kind's, so a signature minted for one kind's route opens nothing on another's. A portrait's tag is `portrait`, which is the whole string portraits signed before there was a second kind, so URLs already issued still verify. URLs are minted only by the subject's own reads, `portraitImages` (`repo/Characters.ts`), `campaignImages` (`repo/Campaigns.ts`) and `toGroup` (`repo/Groups.ts`), and only for an id a read's own predicate returned. Expiry is bucketed (12 h windows, 36 h after the window opens), so every read in a window mints the same URL and the browser cache holds. Every parameter is a plain string, so a malformed request gets the same `NotFound` as a forged, expired, wrong-kind or not-yet-`ready` one. The handler returns an `HttpServerResponse.stream` from `ObjectStorage` with `Cache-Control: private, max-age=<until expiry>, immutable`, `nosniff` and `default-src 'none'`. The client joins the path to its API base (`apiUrl` in `apps/web/src/api/client.ts`); the server never knows its public origin.

Signing is optional per repository. Each repository that mints URLs reads `ImageUrls` with `Effect.serviceOption` at build time (`imageSigner`, wrapped as `portraitSigner`, `campaignImageSigner` and `sharedWorldImageSigner`), so `app.ts` must provide `imageUrls` where that repository is wired. A repository test that builds its own layers proves nothing about that wiring; `portraits.test.ts`, `campaign-images.test.ts` and `shared-world-images.test.ts` check it over the real application. `Layer` memoises by identity, so when the same layer is used bare in one place and with the signer in another, whichever copy builds first is shared by both. The handlers' `Recap`, `Campaigns` and `Groups` are therefore `Layer.fresh(…)` with the signer, while the copies held by the chronicle, Hob and the accept paths stay bare. Hob's `sessionRecap` tool sends a recap to the model, and a signed URL is a bearer capability.
