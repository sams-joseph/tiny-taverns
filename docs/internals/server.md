...
- **The boot line must stay.** `identityFromConfig` in `app.ts` logs `Hosted sign-in is ON` or `OFF` on every start, names the file to set when off, and never prints key material (not a prefix, not a length). `env-file.test.ts` asserts all three. `assistantFromConfig` follows the same pattern for Hob, `storageFromConfig` for object storage (`storage.md`), and `portraitsFromConfig` for portraits, whose OFF line names every missing variable of the four it needs.

## The first background worker: portraits

`apps/server/src/portraits/Portraits.ts` is the only code that outlives a request. A handler hands it a job and returns; the service owns a `FiberSet` (so shutdown interrupts every job) and a `Semaphore` of `PORTRAIT_CONCURRENCY`, and each job, permit wait included, runs under a 150 s timeout. Its state is Postgres, never memory: a row is `generating` until the job commits `ready` or `failed`, and a sweep at boot and every minute marks rows older than any job can live (`STALE_AFTER_SECONDS`) `interrupted`, which is also what makes the sweep safe beside a second instance. The same loop drains `storage_deletion`. It runs whenever storage is on, even with generation off, because a deleted character's files must still go. Scaling past one instance means replacing the in-process hand-off with a `for update skip locked` pickup over `generating` rows.

The image provider is a plain `HttpClient` call (`portraits/ImageModel.ts`, OpenAI-shaped `POST /images/generations`, endpoint-aware body like `assistant/modelConfig.ts`), stubbed in tests at `HttpClient` by `test/support/imageModel.ts`. No provider text reaches a row or the wire; it goes to the log line.

## The signed image route

`GET /portraits/:portraitId/:variant?e=&s=` is the one route serving bytes, and it has no `Authorization` because an `<img>` cannot send the bearer header. The capability is an HMAC over portrait, size and expiry (`portraits/PortraitUrls.ts`, keyed by `PORTRAIT_URL_SECRET`), minted only by `portraitImages` (`repo/Characters.ts`) for a portrait id a read's own predicate returned. Expiry is bucketed (12 h windows, 36 h after the window opens) so a window's reads mint identical URLs and the browser cache holds. Every parameter is a plain string, so a malformed request is the same `NotFound` as a forged, expired or not-yet-`ready` one. The handler returns an `HttpServerResponse.stream` from `ObjectStorage` with `Cache-Control: private, max-age=<until expiry>, immutable`, `nosniff` and `default-src 'none'`. The client joins the path to its API base (`apiUrl` in `apps/web/src/api/client.ts`); the server never knows its public origin.

Signing is optional per repository: each one that mints reads `PortraitUrls` with `Effect.serviceOption` at build time (`portraitSigner`), so `app.ts` must provide `portraitUrls` at that repository's wiring site, and a repository test that builds its own layers proves nothing about that wiring. `portraits.test.ts` checks it over the real application. `Layer` memoises by identity, so when the same layer is used bare in one place and with the signer in another, whichever copy builds first is shared by both. The handlers' `Recap` is therefore `Layer.fresh(Recap.layer)` with the signer, and the chronicle's and Hob's copies stay bare: Hob's `sessionRecap` tool sends a recap to the model, and a signed URL is a bearer capability.
