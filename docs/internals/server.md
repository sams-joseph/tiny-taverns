...
- **The boot line must stay.** `identityFromConfig` in `app.ts` logs `Hosted sign-in is ON` or `OFF` on every start, names the file to set when off, and never prints key material (not a prefix, not a length). `env-file.test.ts` asserts all three. `assistantFromConfig` follows the same pattern for Hob, `storageFromConfig` for object storage (`storage.md`), and `hobImagesFromConfig` for Hob-drawn images, whose OFF line names every missing variable of the four it needs.

## The background worker: Hob-drawn images

`apps/server/src/images/HobImages.ts` is the only code that outlives a request, and it serves every kind of image ([Images](images.md)). A handler hands it a job and returns. The service owns a `FiberSet`, so shutdown interrupts every job, and a `Semaphore` of `PORTRAIT_CONCURRENCY` shared by all kinds. Each job, permit wait included, runs under a 150 s timeout. Its state is Postgres, never memory: a row is `generating` until the job commits `ready` or `failed`. A sweep at boot and every minute marks rows of every kind older than any job can live (`STALE_AFTER_SECONDS`) `interrupted`, which is also what makes the sweep safe beside a second instance. The same loop drains `storage_deletion`. It runs whenever storage is on, even with generation off, because a deleted subject's files must still go. Scaling past one instance means replacing the in-process hand-off with a `for update skip locked` pickup over `generating` rows.

The image provider is a plain `HttpClient` call (`images/ImageModel.ts`): an OpenAI-shaped `POST /images/generations` at the kind's size, with an endpoint-aware body like `assistant/modelConfig.ts`. Tests stub it at `HttpClient` with `test/support/imageModel.ts`. No provider text reaches a row or the wire; it goes to the log line.

## The signed image routes

`GET /portraits/:imageId/:variant?e=&s=`, `GET /campaign-images/:imageId/:variant?e=&s=`, `GET /shared-world-images/:imageId/:variant?e=&s=`, `GET /npc-images/:imageId/:variant?e=&s=` and `GET /battle-map-images/:imageId/:variant?e=&s=` (the `images` group, one endpoint per kind) are the only routes that serve bytes. They have no `Authorization`, because an `<img>` cannot send the bearer header. The capability is an HMAC over `{tag}:{imageId}:{variant}:{expiry}` (`images/ImageUrls.ts`, keyed by `PORTRAIT_URL_SECRET`). The tag is the kind's, so a signature minted for one kind's route opens nothing on another's. A portrait's tag is `portrait`, which is the whole string portraits signed before there was a second kind, so URLs already issued still verify. URLs are minted only by the subject's own reads, `portraitImages` (`repo/Characters.ts`), `campaignImages` (`repo/Campaigns.ts`), `toGroup` (`repo/Groups.ts`), `npcImageOf` (`repo/Npcs.ts`) and `battleMapImages` (`repo/BattleMaps.ts`, for its creator-gated reads and the live table's board in `repo/PlayerTable.ts`), and only for an id a read's own predicate returned. Expiry is bucketed (12 h windows, 36 h after the window opens), so every read in a window mints the same URL and the browser cache holds; losing sight of a subject does not revoke a URL already issued, it lapses. Every parameter is a plain string, so a malformed request gets the same `NotFound` as a forged, expired, wrong-kind or not-yet-`ready` one. The handler returns an `HttpServerResponse.stream` from `ObjectStorage` with `Cache-Control: private, max-age=<until expiry>, immutable`, `nosniff` and `default-src 'none'`. The client joins the path to its API base (`apiUrl` in `apps/web/src/api/client.ts`); the server never knows its public origin.

Signing is optional per repository. Each repository that mints URLs reads `ImageUrls` with `Effect.serviceOption` at build time (`imageSigner`, wrapped as `portraitSigner`, `campaignImageSigner`, `sharedWorldImageSigner` and `npcImageSigner`; `BattleMaps` and `PlayerTable` read `imageSigner` directly), so `app.ts` must provide `imageUrls` where that repository is wired. A repository test that builds its own layers proves nothing about that wiring; `portraits.test.ts`, `campaign-images.test.ts`, `shared-world-images.test.ts`, `npc-images.test.ts` and `battle-maps.test.ts` check it over the real application. `Layer` memoises by identity, so when the same layer is used bare in one place and with the signer in another, whichever copy builds first is shared by both. The handlers' `Recap`, `Campaigns`, `Groups`, `Npcs` and `NpcThreads` are therefore `Layer.fresh(…)` with the signer, while the copies held by the chronicle, Hob, the NPC agent and the accept paths stay bare. `BattleMaps` has one copy, the handlers', with the signer, and the only other reader of a map's picture is `PlayerTable`, also wired with it. Hob's `sessionRecap` tool sends a recap to the model, the NPC agent builds a model's context from an NPC, and a signed URL is a bearer capability.
