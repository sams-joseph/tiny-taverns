...
- **`tailwind-merge` must be told the theme's names** (`packages/ui/src/lib/tw-theme.ts`). With a replaced theme it cannot tell `text-label` (a size) from `text-on-accent` (a colour) and silently drops one; that rendered primary buttons in body colour until configured. `tw-theme.test.ts` parses the `@theme inline` block and fails if the lists drift. The merge config knows only the page-width containers, so a component must set no `max-w-*` default: it would survive beside a consumer's and source order would decide.
- **A spacing name and a container name must differ.** `max-w-*` reads both `--spacing-*` and `--container-*`, and the spacing one wins: with `--spacing-encounters-list` (260px) beside `--container-encounters-list` (360px), `max-w-encounters-list` capped the list at 260 with nothing failing. So a floor is `-min` (`basis-encounters-list-min`) and the cap keeps the bare name.
