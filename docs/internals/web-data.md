...
**The rule for a new write: ask what it changes that is not in its own response.** The instances in the product: a roster line moves `Encounter.creatureCount` and `difficulty` (computed per read), so it names `reads.encounters`, and so does starting or ending a fight, which moves `lastPlayed`; the difficulty also moves with the party and `lastPlayed` names a night, so the encounters read answers `reads.party` and `reads.sessions` as well; a note's attachment moves an encounter's read-aloud, so a note write refreshes the notes; a player's own sheet moves the party list at every seat (`characters/write.ts`'s `ownCharacterWrites`); a combatant's `conditions` is written through to the character (`run/load.ts`'s `combatantWrites` names `reads.party`). Prefer over-naming: a key nobody listens on costs nothing.
