...
- `EncounterRuns.resume` (`POST …/sessions/:s/runs/resume { continuedFrom }`) copies the round, mode, phase, visibility, provenance, the board, the scene with its log of checks, and every combatant. Combatant ids are generated in TypeScript before the insert, the only way the turn marker can carry: `encounter_run_active_combatant_fkey` refuses a marker naming another run's combatant, so `insert … select` could not remap it.
