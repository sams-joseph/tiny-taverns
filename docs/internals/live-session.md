...
- A run that is not a fight has no order: only the asker's own rows are selected, for `seats`, and `order` is empty with `upNext` null. Nothing of the scene is on this read.
