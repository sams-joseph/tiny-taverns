...
- A fight keeps its board: `encounter_run_board` (`0058_encounter_run_boards.ts`), one per run. `start` copies the encounter's map's grid onto it and `resume` copies the predecessor's, so a grid edit on the encounter's page is the next fight's and never a running or finished one's, as `encounter_name` is; the play feature's positions in squares will need a board that does not move under them. The picture is not copied but read through `map_id`, since it is drawn once and never redrawn. `map_id` is `on delete set null`: deleting the encounter takes the map and its picture and leaves the fight its grid on a blank board. It is its own table because a player's recap reads every `encounter_run` column (`runColumns`). `GET …/runs/:r/board` (`BattleMaps.forRun`) is the creator's alone; the runner shows it in a band under the initiative list, closed by default, and no player read carries it.
