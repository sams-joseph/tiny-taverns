...
- Ordinary screens grow the document and the window scrolls. A screen that deliberately owns an internal scroller (the runner, the player table, the sheet, the NPC talk page) declares `staticData: { fill: true }` on its route; the layout reads it through `useMatches`, since the layout renders the route and a prop cannot travel up, and bounds only that content region to the viewport below the same chrome stack.
...
- Every row has a fixed height, written down in `AppShell.tsx`: 44 global, 46 campaign, 76 per-screen bar, 40 tab strip. The bar reserves the subtitle line whether or not a screen has one and never wraps, so every campaign destination shares one content top edge — measured 61 / 89 / 141 before, one number after. The delivered `base.css` gives every `p` a 12px bottom margin, which is why the bar's subtitle carries `mb-0`.
...
- The screen owns its scroller (`fill`) because the sticky columns and the scroll-spy need a top edge that is the screen's rather than the shell's. It cancels the shell's page padding with a negative margin, so it reaches for the same `@container/app` the shell does; two spellings of one edge is how they drift.
...
`hob/useHobPanel.ts` owns open state, `inline`, and the ⌘K/Esc handlers; `HOB_INLINE_MIN` is 1020 and `inline` is `!useIsMobile(HOB_INLINE_MIN)`, so there is one media query. Above it the panel is a 400px sidebar beside the content; below it a scrimmed overlay at `z-scrim`/`z-dialog`. The persistent layout calls `useHobPanel({ initialOpen: false })` once, so nothing is requested until the panel opens, and the panel stays open across navigation and across campaigns. The shell's seam is two props, `onAskHob` and `panel`; a screen with an "ask Hob" control of its own calls `useShowHob()`.
