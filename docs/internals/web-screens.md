...
- The campaign row sources itself: the way home from the route, the name from `campaignAtom`, and, for a creator only, the session badge and the campaign's press from `campaignNightAtom` (the campaign, its open session and its live run, without the rest of the view). `campaign/campaignRow.test.tsx` reads the destinations out of the rendered row and visits each.
...
- `useCampaignAct` (`campaign/act.tsx`) computes the campaign's press once, asking the session and the run separately: a live run, no session, or a night open with nothing on the table. The run is undefined in the last case too, which is what a two-way branch gets wrong. The campaign row and the Overview's card both call it; on the fight it would send you back to, the press is absent. Opening and finishing a night go through `session/start.ts` and `session/finish.ts`; see [Live session](live-session.md).
