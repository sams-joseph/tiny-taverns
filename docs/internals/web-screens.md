...
- A screen reaches the shell at one point: `TopBar` (`shell/TopBar.tsx`), rendered anywhere in the screen and portalled into a slot inside that sticky stack. Nothing travels up as a prop, and the rows need no separately maintained `top` offsets.
