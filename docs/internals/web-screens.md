...
`shell/AppShell.tsx` draws a global row and a campaign row that exists exactly when the route names a campaign. `shell/ShellLayout.tsx` is its only mount, so the header, both rows, the bar's slot and the Hob panel are the same nodes across navigation; `shell/ShellLayout.test.tsx` pins that by node identity and fails if any other file imports `AppShell`. Neither row is sticky or on the layering scale; they sit above the scrolling column, and `main` is a `@container`.

- A screen reaches the shell at one point: `TopBar` (`shell/TopBar.tsx`), rendered anywhere in the screen and portalled into a slot the shell owns. Nothing travels up as a prop. The slot is the sticky element at `z-chrome`, not the header inside it, because a sticky box sticks within its parent and a parent exactly as tall as the header gives it no room.
- A screen that owns its scroller (the runner, the player table, the sheet, the NPC talk page) declares `staticData: { fill: true }` on its route; the layout reads it through `useMatches`, since the layout renders the route and a prop cannot travel up.
...
- The campaign row is navigation and nothing else. Its one verb — _Start session_ / _Start an encounter_ / _Back to the fight_ — is in the per-screen bar's action cluster, sourced from the route the same way (`shell/TopBar.tsx`), and absent on the Overview, whose _Tonight_ card already carries it. It sits outside the cluster's scroller so a crowded screen cannot hide it.
- The row's lead cell (name, world chip, badge) is a fixed `w-96` at `@5xl` and above, so the first tab starts at the same x whatever the campaign is called; the name truncates inside it. Below `@5xl` the chip drops to its icon, the badge goes, the cell shrinks to its contents and the tab strip scrolls.
