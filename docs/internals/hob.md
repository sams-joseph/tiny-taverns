...
This page covers the server side of Hob (`apps/server/src/assistant/`), the conversation and accept repositories behind it, the five toolkits, what the panel suggests where, configuration, how to diagnose a model that "never calls a tool", and the NPC agent that shares Hob's loop and model. The panel's layout is in [Web screens](web-screens.md); the predicates Hob composes are in [Visibility](visibility.md).
...
One transaction, turn locked `for update` first: a double-tapped save is one row and one `Conflict`; a turn that proposed nothing is `NotFound` about the proposal. `materialise` has six targets: `note`, `beat` (session resolved at accept time; none is a `Conflict`), `encounter` with its kind and prep (tactics, treasure, a skill challenge's or hazard's numbers), the setting line its battle map is drawn from (the accept handler starts the draw after commit, [Images](images.md)) and its roster, all through one `Encounters.create` whose `creatures` go line by line through `EncounterCreatures.create`, the path the form's create takes, so a creature the campaign can no longer use refuses the whole keep; the accept names no `ready`, so the kept encounter is a draft, `character` (through `Characters.createOwn`, owned by the accepting credential), and the two Shared World kinds behind `acceptSharedWorld`. `acceptDraft` is the account-scoped accept, with two targets: a `character` through `Characters.createCore`, or a `campaign` through `Campaigns.createStandalone` (or `Campaigns.create` into the Shared World the proposal named) with the payload `campaignCreateFrom` builds, the same function `NewCampaignDialog` calls, so the form and Hob start a campaign with the same fields. The asker becomes its creator, the row carries `origin = 'assistant'` and the turn (the columns `0001` gave `campaign`; no migration was needed), and the `meHob.accept` handler starts the one cover draw after commit, as `POST /campaigns` does. The world was resolved from `Groups.mine` when the proposal was made and the create checks it again, so a world archived or left since refuses the keep with a `NotFound`. Which target a caller can reach is decided by the thread's reach, not a check here: a `character` proposal exists only in a player-reach or account thread and a `campaign` proposal only in an account thread, so a campaign's own thread can never accept either. Accepted rows take the column default for `visibility`. Discard is not built and not faked; `hob-proposals.test.ts` counts the content tables either side of a proposal.
...
## Five toolkits, because a toolkit is what the model is shown

A tool bound to a handler that always refuses is still a tool the model spends a round reaching for. So `toolkit.ts` has five toolkits, not one narrowed at the handler:

- `HobToolkit` (campaign creator): the campaign reads plus `getNpc`, the two read-only Shared World tools, `proposeNpcAwareness`, and `proposeNote` / `proposeBeat` / `proposeEncounter`. `proposeNote` takes an optional `category`, carried on the proposal to the accept; the NPC's `proposeCampaignNote` leaves a note uncategorised. `proposeEncounter` takes the challenge's numbers as flat parameters and `challengeFrom` sorts them by kind, refusing in words the model can act on (a fight with no creatures, numbers the kind does not take, half a challenge) rather than dropping what it said. No other toolkit reads or writes an encounter, so the kind and prep reach none of them.
- `SharedWorldToolkit`: history search, summary, Story So Far sources, the world's campaigns, played nights, `nightStory`, and `proposeSharedWorldEntry` / `proposeStorySoFar`. It knows the played nights across the world, of each only what its DM shared, and nothing unplayed ([Shared Worlds](shared-worlds.md)). `hob-group.test.ts` plants sentinels in notes, planned encounters, prep lines and draft threads, and in a played night's hidden fight, running fight, DM-only beat and DM-only prep line, and asserts zero occurrences in any captured request.
...
- The core drafting toolkit (`coreToolkitOver`, `/me/hob` with `intent: "character"`, the character create screen's composer): the player's without `searchCampaign`, over the core rules (`Options.core`, `Spells.forDraft(null, …)`, both `coreRulesUsable`). It has no campaign or Shared World tool to reach with, and `Hob.askDraft` holds no campaign repository. `hob-draft.test.ts` checks the tool list, the vocabulary and that a creator's campaign reaches no request byte.
- The account panel's (`accountToolkitOver`, `/me/hob` with no `intent`, the panel on every screen outside a campaign or Shared World): the core drafting toolkit plus `proposeCampaign`, built per request over the asker's Shared Worlds (`Groups.mine`, the list the Shared Worlds screen shows). `sharedWorld` is a world name through `nameSchema`, so the grammar holds the model to the asker's worlds; with none it is free text the handler refuses unless absent. It reads no campaign's record and offers no campaign content. `hob-account.test.ts` checks the tool list, the world names and the keep.

`playerHandlersFor`, `coreHandlersFor` and the account panel's handlers share one `draftingHandlersFor`, so every drafting surface composes a character with the same code over a different vocabulary and spell list.

Where campaign drafting belongs was decided per toolkit. Only the account panel has it: a new campaign is an account's act, not a campaign's, and its accept lives on `/me/hob`. The creator's campaign toolkit plans inside one campaign and its accept files into that campaign; the player's and the core drafting toolkits are the character composer and nothing else; Shared World Hob keeps the world's canonical record, and any member may accept there; the NPC agent speaks as a character. From any of those the panel is one navigation away from the account's.
...
The redraft loop is `promptFor`'s `offered()`: a saved `turn.proposal` is rendered back into the next prompt (labels, the ranking rather than six cells, the appearance line, an encounter's `creatureId`s, kind and prep), so "make her a ranger instead" reaches a model that can see the druid it wrote. Accept comes before corrections; after it, every edit is an ordinary owner PATCH.
...
`apps/web/src/hob/conversation.ts` is the only web file that talks to the assistant. Its scope is read off the route in `Hob.tsx`: a campaign its creator is at (`hob`), a Shared World (`sharedWorldHob`), or the account on every other screen (`meHob`, no `intent`). A player's campaign is no scope, because the panel's verbs there are the creator's. `callsFor` picks the one API group for a scope, including the keys an accept invalidates; a kept campaign or character is opened through `onKept`, which is how the panel navigates without knowing the router.

What the empty panel suggests is `hob/suggestions.ts`, the one mapping from place to starters: a campaign's delivered starters, the Shared World's history starters, and on account screens only the two drafts its toolkit can make, ordered by the screen (a campaign first on the Campaigns list, a character first on the roster) and worded by what the screen already read (_my first_, _another_, or neither when the count is unknown). Counts come only from the atom the screen behind the panel already holds, read only while the panel is open. `suggestions.test.ts` pins that no starter offers what its toolkit cannot do. Nothing is requested until the panel is opened, and status is re-asked on each open. The "Knows" strip draws only what `HobStatus` vouched for; `hob.fixtures.ts`'s sample thread is for tests and must never reach a screen. Omit an optional key rather than sending `undefined` (`threadId: undefined` arrives as null and `Schema.optional` refuses it). The panel is shadcn's `sidebar` over `sheet`. Docked, it is a full-height column of `HobFrame` beside the whole shell; overlaid, `sheet`'s `container` prop portals it into `HobRegion` so it covers the content under the bars, not the app.
