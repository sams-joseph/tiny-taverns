...
Core module: `effect/unstable/ai`. In the current `effect-smol` checkout, the provider packages present are `@effect/ai-openai`, `@effect/ai-openai`, `@effect/ai-openai-compat`, and `@effect/ai-openrouter`.
...
const chat =
  yield * persistence.getOrCreate("conversation-1", { timeToLive: "30 days" });
...
interface Service {
  ...
  ) => Effect<Array<number>, AiError.AiError>;
  readonly truncate: (
    input: Prompt.RawInput,
    tokens: number,
