...
const makeMockChatRepo = (
  updatedMessagesRef?: Ref.Ref<ReadonlyArray<typeof Chat.Message.Type>>,
) =>
  Layer.mock(ChatRepo)({
...
describe("ChatRunManager", () => {
  ...
  it.live(
    "startGeneration fails with GenerationInProgressError when already running",
    () => {
      const slowAi = Layer.mock(AiModels)({
        use: (_model) => (effect) =>
          withLanguageModel(effect, {
  ...
  it.live(
    "interrupt fails active stream with interrupt-only cause",
    () => {
      const slowAi = Layer.mock(AiModels)({
        use: (_model) => (effect) =>
          withLanguageModel(effect, {
  ...
  it.live(
    "interrupted run invalidates the run stream entry",
    () => {
      const slowAi = Layer.mock(AiModels)({
        use: (_model) => (effect) =>
          withLanguageModel(effect, {
  ...
  it.live(
    "closing manager scope interrupts an active generation",
    () => {
      let finalized = false;
      const slowAi = Layer.mock(AiModels)({
        use: (_model) => (effect) =>
