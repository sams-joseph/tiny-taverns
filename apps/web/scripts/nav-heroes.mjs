 ...
 *
 * Each picture is 2:1 and is drawn in a tile 12rem wide beside the panel's
 * links, or across the panel when it stacks on a narrow screen; the two widths
 * are that tile at 1x and 2x, and the larger also covers the stacked tile.
...
for (const [name, source] of [
  ...
  for (const width of [384, 768]) {
    ...
    const info = await sharp(source)
      .resize({ width, height: width / 2, fit: "cover" })
