import { HealthStatus } from "@taverns/api";
 ...
 *
 * `HealthStatus` now lives in `@taverns/api`: it is part of the wire contract,
 * so the declaration the derived client decodes is the one the handler produces.
 */
...
>()("Health") {
  ...
  static readonly layer = Layer.succeed(this)({
    check: Effect.sync(() => new HealthStatus({ status: "ok", uptime: process.uptime() })),
  });
}
