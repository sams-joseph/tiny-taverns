 ...
 * File: 5e-SRD-Magic-Items.json.
 *
 * Rebuild recipe:
 *   node - <<'NODE'
 *   const fs = require("node:fs");
 *   const src = "/path/to/5e-database/src/2014/en/5e-SRD-Magic-Items.json";
 *   const data = JSON.parse(fs.readFileSync(src, "utf8"));
 *   if (data.length !== 362) throw new Error(`expected 362, got ${data.length}`);
 *   // Write this wrapper around JSON.stringify(data, null, 2).
 *   NODE
 *
 ...
 * THIRD_PARTY_NOTICES.md and rules_source_document rows written by magic-item:import.
 */
...
export const MAGIC_ITEM_RAW = JSON.parse(String.raw`[
  {
