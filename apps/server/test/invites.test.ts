...
describe("what an invitation grants", () => {
  it("previews before there is an account, and names the group, its owner and the table", async () => {
    ...
    const issued = await mint(fixture.campaign, "Ilse");
  ...
  it("writes a group membership and a participation, and nothing that could be mistaken for creator-ness", async () => {
  ...
  it("grants at the invitation's campaign, because there is nowhere to name another", async () => {
    ...
    const issued = await mint(fixture.otherTable, "Elsewhere");
...
describe("the first player actor, and what it reaches", () => {
  ...
  it("cannot write anything, though it can read the shared half", async () => {
    ...
    const issued = await mint(fixture.campaign, "Tam");
  ...
  it("cannot reach the three live repositories the DM gate protects", async () => {
    ...
    const issued = await mint(fixture.campaign, "Vess");
...
describe("the lifetime rules", () => {
  it("is single-use — a second account gets nothing, the same account gets the same answer", async () => {
    const issued = await mint(fixture.campaign, "Wen");
  ...
  it("expires, and says nothing more than an invented token would", async () => {
    const issued = await mint(fixture.campaign, "Zed");
  ...
  it("is revocable after acceptance, which is the remedy for a forwarded link", async () => {
    ...
    const issued = await mint(fixture.campaign, "for Ilse");
  ...
  it("refuses to revoke an invitation of another group, by id", async () => {
...
describe("the tables I am at", () => {
  it("shows a player their table only once the DM has shared it", async () => {
    ...
    const issued = await mint(fixture.otherTable, "Private table");
...
describe("what an invitation cannot do to the campaign it names", () => {
  it("leaves the creator's participation unrepresentably intact", async () => {
  ...
  it("cannot be minted as a creator invitation, because there is nothing to mint one with", async () => {
  ...
  it("stores no token in plaintext, and shows one exactly once", async () => {
    const issued = await mint(fixture.campaign, "Secrecy");
